...
namespace Cyberbit.TaskManager.Server.Controllers
{
    ...
    public class TasksController : ControllerBase
    {
        ...
        [HttpGet("doneAll/{employeeId}")]
        [Authorize]
        public async Task<ActionResult<List<TaskDto>>> GetTaskSByEmployee([FromRoute] int employeeId)
        {
            if (employeeId <= 0)
            {
                var errMsg = $"Failed to get Task with invalid employee id '{employeeId}'";
                _logger.LogError(errMsg);
                return BadRequest(errMsg);
            }

            var allTasks = await _TasksBl.GetTasksByEmployeeId(employeeId);
            if (allTasks == null)
            {
                var errMsg = $"Failed to get Task with id '{employeeId}. Task not found'";
                _logger.LogError(errMsg);
                return NotFound(errMsg);
            }
            var allTasksDto = _autoMapper.Mapper.Map<List<TaskDto>>(allTasks);
